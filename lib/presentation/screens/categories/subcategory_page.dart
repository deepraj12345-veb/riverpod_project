...
class _SubcategoryPageState extends ConsumerState<SubcategoryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    categoriesAsync.whenData((categories) {
      try {
        ...
        if (subcategories.isEmpty) {
          final fakeNames = FakeData.subcategories[widget.categoryName] ?? [];
          subcategories = fakeNames
              .map(
        ...
        // Fallback if category not found in API but exists in fake data
        final fakeNames = FakeData.subcategories[widget.categoryName] ?? [];
        subcategories = fakeNames
            .map(
              (name) => SubcategoryEntity(id: name, name: name, categoryId: ''),
            )
            .toList();
      }
    });
    ...
    final allSubs = [
      SubcategoryEntity(id: 'all', name: 'All', categoryId: ''),
      ...subcategories,
    ];
    ...
    final finalProducts = displayedProducts.isEmpty
        ? categoryProducts
        : displayedProducts;

    final cartItems = ref.watch(cartProvider);
    final cartTotalItems = cartItems.fold(
      0,
